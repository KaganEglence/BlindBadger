...
check_keywords_list = ["login", "admin", "username",
                       "password", "authentication", "management", "machine"]


def FinPortScanner(target_ip):
...
def take_Screenshot(url):
    try:
        screenshot_name = " ./images/Screenshot-" + \
            str(url).split("//")[1] + ".png"
        html_to_png_command = "wkhtmltoimage -q " + str(url) + screenshot_name

        subprocess.check_output(html_to_png_command,
                                shell=True, stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError:
        pass


def custom_Requester(target, port):
    ...
    try:
        ...
        redirected_url = ""
        pass
    custom_Requester.url = url
    keys_of_url = redirected_url
    founded_word = ""
    for word in check_keywords_list:
        if source_code.lower().find(word) != -1 or keys_of_url.lower().find(word) != -1:
            founded_word = founded_word + "," + word
    ...
    try:
        if redirected_url != "":
            redirect_of_response = redirected_url
            return {"ip": str(target), "redirected_site": str(redirect_of_response), "port": int(port), "source_code_check": str(result_of_check)}

    except (requests.Timeout, socket.timeout, AttributeError):
        pass


def with_ip_adress(target, port):
    response_of_analsys = custom_Requester(target, port)
    return response_of_analsys


def scan_with_ip(target_ip):
    for target in IPNetwork(target_ip):
        try:
            socket.gethostbyaddr(str(target))
            FinPortScanner(str(target))
            for one_port in all_open_ports:
                response_for_db = custom_Requester(target, one_port)
                result = response_for_db
                print("Target:" + str(result["ip"]) + " ## Redirected Url:" + str(result['redirected_site']) +
                      " ## Port:" + str(result['port']) + " ## Login Page Check Result:" + str(result['source_code_check']))
...
def main():
    ...
    parser.add_argument("-t", "--target", type=str,
                        help="Target Ip for Scan(Example: 1.2.3.0/24 , 1.2.3.0)")
